...
const Reviews = () => {
  ...
  const handleclick = (e) => {
    setChoice(e.target.id == 1 ? "read" : "give");
  }
  return (
    <section>
      ...
      <div className="text-center">
        <ButtonGroup size="large" className="bg-dgreen" variant="contained">
          <Button id="1" className={`${choice === "read" ? "bg-orange" : null}`} color="warning" onClick={handleclick} >Read</Button>
          <Button id="2" className={`${choice === "give" ? "bg-orange" : null}`} color="warning" onClick={handleclick} >Give</Button>
        </ButtonGroup>
      </div>
